    ...
    def detector(self):
        ...
        try:
            obj_array = rospy.wait_for_message(
                "/objects", ImageMessage, timeout=5)
        except Exception:
            cube_list.clear()
            return cube_list
        
        for i in range(len(obj_array.x)):
            list1=[]
            list1.append(obj_array.name[i])
            list1.append([obj_array.x[i],obj_array.y[i]])
            cube_list.append(list1) 
